...
= Filtered tasks REST API
REST API to manage resourceName

The project **filteredTasksRestAPI** is a Bonita REST API extension for **Bonita 7.12.9** version  **community** written in `groovy`
...
[source,bash]
----
git clone https://your.github.com/filteredTasksRestAPI.git
cd filteredTasksRestAPI/
mvn package
----

The build should produce a zip archive under the `target/` folder named `filteredTasksRestAPI-1.0.0-SNAPSHOT.zip`
...
Take the built artifact `filteredTasksRestAPI-1.0.0-SNAPSHOT.zip` and upload it to your Bonita platform (see https://documentation.bonitasoft.com/bonita/${shortBonitaVersion}/rest-api-extensions#toc7)
